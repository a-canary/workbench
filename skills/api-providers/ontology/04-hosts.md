...
|---|---|---|---|---|---|
| **arc-llm-proxy** (switchboard) | `http://127.0.0.1:8091/v1` (LAN `YOUR-OPERATOR-BOX-IP:8091`) | operator box | Bearer `FACTORY_KEY` (`source arc-llm-proxy/deploy/.keys.env`) | `switchboard.default.json` + `deploy/switchboard.local.json` (local wins) | ✅ up |
...
- Switchboard `endpoints`: currently **only `e103`** (`http://YOUR-GPU-BOX-IP:1234`, Bonsai-27B). The README documents `spawn`-type and multi-endpoint ladders (e.g. `eclaude` → cli-proxy) — not wired in the live deploy.
...
|---|---|---|---|
| GPU box (4060) | `YOUR-GPU-BOX-IP:1234` (llama.cpp) | Bonsai-27B-Q1_0.gguf — the factory workhorse | ❌ down (human-gate `hg-arc-director-gpu-box-103-down-1787548017`) |
| Operator box | `YOUR-OPERATOR-BOX-IP` | runs both routers | ✅ up |
| LM Studio box | `YOUR-LM-STUDIO-IP:1234` | Bonsai-27B (64k), qwen3-4b, qwen3.5-9b distills | ? not probed |
